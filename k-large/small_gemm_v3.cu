...
__global__ void Kernel(float4* matrix_a, float4* matrix_b, float4* matrix_c) {
    ...
    unsigned bxdiv2 = by / 2;

    unsigned index_a = tx + bxmod2 * (128/4) + bxdiv2 * 16;
    unsigned index_b = ty + by * 16 + bxmod2 * 64;

    unsigned index_c0 = tx + bx * 16 + ty * (256 / 4) * 4 + by * (256 / 4) * 64;
    unsigned index_c1 = index_c0 + (128/2);
    unsigned index_c2 = index_c1 + (128/2);
    unsigned index_c3 = index_c2 + (128/2);
...
int main() {
    ...
    cudaMemcpy(C.data(), Cd, size * 2, cudaMemcpyDeviceToHost);

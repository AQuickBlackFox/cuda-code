...
__global__ void Kernel(float4* matrix_a, float4* matrix_b, float4* matrix_c) {
    ...
    unsigned bxmod2 = bx % 4;
    unsigned bxdiv2 = bx / 4;

    unsigned index_a = tx + bxmod2 * (128 / 4) * 32 + bxdiv2 * 16;
    unsigned index_b = ty + by * 16 + bxmod2 * (128 / 4) * 32;
...
int main() {
    ...
    Kernel<<<dim3(8,2,1), dim3(16,16,1)>>>(Ad, Bd, Cd);
    }

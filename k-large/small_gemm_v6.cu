...
__global__ void Kernel(float4* matrix_a, float4* matrix_b, float4* matrix_c) {
    ...
    unsigned bxmodk = bx % split_k;
    unsigned bxdivk = bx / split_k;

    unsigned index_a = tx + bxmodk * (128 / 4) * 32 + bxdivk * 16;
    unsigned index_b = ty + by * 16 + bxmodk * (128 / 4) * 32;

    unsigned index_c0 = tx + bxdivk * 16 + ty * (128 / 4) * 4 + by * (128 / 4) * 64;
    ...
    for(size_t i = 0; i < (128 / split_k); i++) {
